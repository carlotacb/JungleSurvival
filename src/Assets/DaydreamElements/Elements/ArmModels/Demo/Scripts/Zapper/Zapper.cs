...
namespace DaydreamElements.ArmModels {
  ...
  public class Zapper : GvrBasePointer {
    ...
    private void SpawnProjectile() {
      ...
      if(Physics.Raycast (shootRay, out shootHit, 1000f, shootableMask))
      {
        GameObject projectile = GameObject.Instantiate(projectilePrefab);
        gunAudio.Play();
        projectile.transform.position = barrel.position;
        shootHit.point = barrel.position;
        projectile.transform.LookAt(reticle);
        ZombieDeath zombieHealth = shootHit.collider.GetComponent <ZombieDeath> ();
